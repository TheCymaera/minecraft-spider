...
private val blocks by lazy {
    currentPlugin.getResource("block_colors.json")
        ?.bufferedReader()
        ?.use { it.readText() }
        ?.parseJSONColors()
...
private val blocksWithBrightness by lazy {
    mutableMapOf<Color, Pair<Material,Int>>().apply {
        ...
        // This inner loop is modifying the map while iterating, which can be risky.
        // Creating a list of changes to apply after is safer.
        val changes = mutableMapOf<Color, Pair<Material, Int>>()
        for ((color, material) in this) {
            val id = material.first.key.toString()
            if (id.endsWith("_log")) {
                val woodMaterial = Material.matchMaterial(id.replace("_log", "_wood"))
                if (woodMaterial != null) {
                    changes[color] = woodMaterial to material.second
                }
            }
        }
        this.putAll(changes)
...
private val blockToColor by lazy {
