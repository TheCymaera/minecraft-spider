...
object RobotLegModel {
    ...
    val TIBIA = parseModelFromCommand(
        ...
        pieces.forEach { if (it.block.material == Material.SMOOTH_QUARTZ) it.block = Material.BLACK_SHULKER_BOX.createBlockData() }
    }
    ...
    val TIP = parseModelFromCommand(
        ...
        pieces.forEach { if (it.block.material == Material.SMOOTH_QUARTZ) it.block = Material.BLACK_SHULKER_BOX.createBlockData() }
//        pieces.forEach { if (it.block.material == Material.SMOOTH_QUARTZ) it.block = Material.AIR.createBlockData() }
//        pieces = pieces.filter { it.block.material != Material.SMOOTH_QUARTZ }
    }
}
