...
fun spiderModel(spider: Spider, bodyModel: List<SpiderModelPiece>): Model {
    ...
    for ((index, info) in bodyModel.withIndex()) {
        ...
        model.add(id, blockModel(
            ...
            update = {
                it.applyTransformationWithInterpolation(info.transformation)

                it.block = if (!info.cloak) {
                    info.block
                } else {
                    val relative = info.transformation.transform(Vector4f(.5f, .5f, .5f, 1f))
                    val pieceLocation = spider.location.clone().add(relative.x.toDouble(), relative.y.toDouble(), relative.z.toDouble())
    ...
    for ((legIndex, leg) in spider.body.legs.withIndex()) {
        ...
        for ((segmentIndex, segment) in chain.segments.withIndex()) {
            ...
            model.add(Pair(legIndex, segmentIndex), lineModel(
                ...
                update = {
                    val cloak = spider.cloak.getPiece(legIndex to segmentIndex, location)
